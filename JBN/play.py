...
r2 = 2          # middle_layer_node
param_num = (m - 2) * (m - 1) / 2 * r2 ** 2 + (3 * m - 3) * r2 + 2
folder_name = 'dense-' \
              + str(tpn) + '_points-' \
              + str(m) + '_layers-' \
...
def jbn(learning_rate):
    ...
    fig_title = folder_name + '/dense_' + str(learning_rate) + '.eps'
    plt.savefig(fig_title)
    return loss_list[-1]


def main():
    # lr_list = [2e-2, 1.6e-2, 1.4e-2, 1.2e-2, 1e-2, 8e-3, 6e-3]
    # lr_list = [4e-2, 2e-2, 1.8e-2]
    create_dir(folder_name)
    lr_list = [4e-3, 8e-3, 1e-2, 2e-2, 3e-2, 4e-2 ]
    ...
    plt.savefig(folder_name + '/dense__loss_vs_lr2.eps')
    print
    print 'r2 = ' + str(r2)
